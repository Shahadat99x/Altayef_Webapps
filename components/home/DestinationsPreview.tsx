...
export function DestinationsPreview({ countries }: { countries: Country[] }) {
    // Sort logic handled in parent or here? The requirements said "Saudi First featured".
    // We'll assume the list passed in is already sorted or we sort it here simple.
        ...
        <Section variant="white" className="relative overflow-hidden">
            ...
            {countries.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {sortedCountries.slice(0, 6).map((country) => {
                        ...
                        return (
                            <Card
                                ...
                                hoverEffect
                                className={`group relative min-h-[200px] flex flex-col justify-between p-6 border-slate-200 ${isSaudi ? 'ring-2 ring-blue-600 ring-offset-2' : ''}`}
                            >
                                <div className="mb-4">
                                    <div className="w-12 h-12 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center">
                                        {/* Generic Globe/Flag Icon */}
                    ...
                    {countries.length > 5 && (
                        <Link href="/countries" className="group relative overflow-hidden rounded-2xl aspect-[4/5] md:aspect-auto bg-slate-50 border-2 border-dashed border-slate-200 hover:border-blue-500 transition-colors flex flex-col items-center justify-center text-center p-8">
