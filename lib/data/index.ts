...
export async function getAdminByEmail(email: string): Promise<Admin | null> {
  ...
  if (!doc) return null
  return AdminSchema.parse(doc)
}

export async function createAdmin(data: Partial<Admin>): Promise<string> {
  const db = await getDb()
  const admin = {
    ...data,
    createdAt: new Date(),
  ...
  const validAdmin = AdminSchema.omit({ _id: true }).parse(admin)
  const result = await db.collection('admins').insertOne(validAdmin)
  return result.insertedId.toString()
}
